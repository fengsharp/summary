...
void xml_node_append_attr(XmlNode *node, const char *name, const char *value)
{
    XmlAttr *attr = NULL;
    XmlAttr *iter = NULL;
    return_if_fail(node != NULL && (node->type == XML_NODE_PI || node->type == XML_NODE_NORMAL) && name != NULL && value != NULL);
...
int main(int argc, char* argv[])
{
    ...
    XmlNode* child1Text = xml_node_create_text("child1-text");
    xml_node_append_child(child1, child1Text);

    XmlNode* child2Text = xml_node_create_text("child2-text");
    xml_node_append_child(child2, child2Text);
